...
public class GameRender {
    ...
    public void render(float delta) {
        ...
        if (myWorld.isStateNinjaChosen()) {
            myWorld.correct.render(batcher);
            myWorld.wrong.render(batcher);
            if (myWorld.choseCorrect)
                myWorld.plusOneSecond.draw(batcher);
        }
    ...
    private void displayRunningText() {
        AssetsLoader.font.setColor(Color.WHITE);
        AssetsLoader.font.setScale(0.5f, 0.5f);
        AssetsLoader.font.draw(batcher, "LOOK CAREFULLY!", 355, 220);
    }
    ...
    public void setupTween() {
        ...
        Timeline.createParallel().push(text).delay(0.5f).push(button).setCallback(callback).start(tweenManager);
    }
    ...
    public void tweenForChoosingCorrectly() {
        Timeline.createSequence()
                .push(Tween.set(myWorld.plusOneSecond, SpriteAccessor.OPACITY).target(1))
                .beginParallel()
                .push(Tween.to(myWorld.plusOneSecond, SpriteAccessor.OPACITY, 2f).target(0).ease(Quart.OUT))
